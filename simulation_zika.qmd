...
for (i in 1:T+1){
  ...
  out <- as.data.frame(out)
  out$incidence <- parameters["sigma_H"] * out$E_H * parameters["p"]
  out$cumsum <- cumsum(out$incidence)
  
  df_param[i,]$cumul <- out$cumsum[147]
  df_param[i,]$dist_cumul <- (out$cumsum[147]-928)^2 
  df_param[i,]$dist_incidence <- (out$incidence[147]-1)^2
  
  #df_serie <- cbind(df_serie, out$incidence)
}


df_param$dist_carre <- df_param$dist_cumul+df_param$dist_incidence
df_param = df_param[order(df_param$dist_carre, decreasing = FALSE),]
