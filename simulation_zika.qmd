...
$Incidence = \sigma * E * p$  (nbr indiv qui passent de E à I par jour)
```{r}
...
```{r}
outA$incidence <- parameters["sigma_H"] * outA$E_H * parameters["p"]
```

```{r}
plot(outA$time_sim, outA$incidence*54500,
     type = "l", lwd = 2,
     xlab = "Temps (jours)",
     ylab = "Incidence journalière")
...
summary(abc_n)
hist(abc_n)
# plot(abc_n, param = df_param[,.(beta,theta,p)])
```
...
**La matrice V des transitions hors des compartiments infectieux :**   
$$ V = \begin{bmatrix}
\delta E \\
p \delta E - \gamma_I I \\
